...
class ModelInstall extends \Core\Model {
	public function database($data) {
		...
		$file = DIR_APPLICATION . 'opendeal.sql';
		...
		if ($lines) {
			...
			$db->query("INSERT INTO `" . $data['db_prefix'] . "setting` SET `group` = 'config', `key` = 'config_url', value = '" . $db->escape(HTTP_OPENDEAL) . "'");
			...
			$db->query("UPDATE `" . $data['db_prefix'] . "deal` SET `viewed` = '0'");
		}		
	}	
}
?>