...
class ModelDesignLayout extends \Core\Model {
	public function addLayout($data) {
		...
		if (isset($data['layout_route'])) {
			foreach ($data['layout_route'] as $layout_route) {
				$this->db->query("INSERT INTO #__layout_route SET layout_id = '" . (int)$layout_id . "', store_id = '" . (int)$layout_route['store_id'] . "', route = '" . $this->db->escape($layout_route['route']) . "'");
			}	
		}
	}
	...
	public function deleteLayout($layout_id) {
		...
		$this->db->query("DELETE FROM #__layout_route WHERE layout_id = '" . (int)$layout_id . "'");
		$this->db->query("DELETE FROM #__category_to_layout WHERE layout_id = '" . (int)$layout_id . "'");
		$this->db->query("DELETE FROM #__product_to_layout WHERE layout_id = '" . (int)$layout_id . "'");
		$this->db->query("DELETE FROM #__information_to_layout WHERE layout_id = '" . (int)$layout_id . "'");		
	}
