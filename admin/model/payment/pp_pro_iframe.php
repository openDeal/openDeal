<?php
class ModelPaymentPPProIframe extends \Core\Model {
	public function install() {
		$this->db->query("
			CREATE TABLE IF NOT EXISTS `#__paypal_iframe_order` (
		...
		$this->db->query("
			CREATE TABLE IF NOT EXISTS `#__paypal_iframe_order_transaction` (
	...
	public function uninstall() {
		//$this->db->query("DROP TABLE IF EXISTS `#__paypal_iframe_order_transaction`;");
		//$this->db->query("DROP TABLE IF EXISTS `#__paypal_iframe_order`;");
	}

	private function getTransactions($paypal_iframe_order_id) {
		$qry = $this->db->query("SELECT `ot`.*, ( SELECT count(`ot2`.`paypal_iframe_order_id`) FROM `#__paypal_iframe_order_transaction` `ot2` WHERE `ot2`.`parent_transaction_id` = `ot`.`transaction_id` ) AS `children` FROM `#__paypal_iframe_order_transaction` `ot` WHERE `paypal_iframe_order_id` = '" . (int)$paypal_iframe_order_id . "'");
	...
	public function totalCaptured($paypal_iframe_order_id) {
		$qry = $this->db->query("SELECT SUM(`amount`) AS `amount` FROM `#__paypal_iframe_order_transaction` WHERE `paypal_iframe_order_id` = '" . (int)$paypal_iframe_order_id . "' AND `pending_reason` != 'authorization' AND (`payment_status` = 'Partially-Refunded' OR `payment_status` = 'Completed' OR `payment_status` = 'Pending') AND `transaction_entity` = 'payment'");

		return $qry->row['amount'];
	}

	public function totalRefundedOrder($paypal_iframe_order_id) {
		$qry = $this->db->query("SELECT SUM(`amount`) AS `amount` FROM `#__paypal_iframe_order_transaction` WHERE `paypal_iframe_order_id` = '" . (int)$paypal_iframe_order_id . "' AND `payment_status` = 'Refunded'");

		return $qry->row['amount'];
	}

	public function totalRefundedTransaction($transaction_id) {
		$qry = $this->db->query("SELECT SUM(`amount`) AS `amount` FROM `#__paypal_iframe_order_transaction` WHERE `parent_transaction_id` = '" . $this->db->escape($transaction_id) . "' AND `payment_type` = 'refund'");

		return $qry->row['amount'];
	}

	public function getOrder($order_id) {
		$qry = $this->db->query("SELECT * FROM `#__paypal_iframe_order` WHERE `order_id` = '" . (int)$order_id . "' LIMIT 1");
	...
	public function updateOrder($capture_status, $order_id) {
		$this->db->query("UPDATE `#__paypal_iframe_order` SET `modified` = now(), `capture_status` = '" . $this->db->escape($capture_status) . "' WHERE `order_id` = '" . (int)$order_id . "'");
	}

	public function updateTransaction($transaction) {
		$this->db->query("
			UPDATE #__paypal_iframe_order_transaction
	...
	public function getOrderId($transaction_id) {
		$qry = $this->db->query("SELECT `o`.`order_id` FROM `#__paypal_iframe_order_transaction` `ot` LEFT JOIN `#__paypal_iframe_order` `o`  ON `o`.`paypal_iframe_order_id` = `ot`.`paypal_iframe_order_id`  WHERE `ot`.`transaction_id` = '" . $this->db->escape($transaction_id) . "' LIMIT 1");
	...
	public function updateRefundTransaction($transaction_id, $transaction_type) {
		$this->db->query("UPDATE `#__paypal_iframe_order_transaction` SET `payment_status` = '" . $this->db->escape($transaction_type) . "' WHERE `transaction_id` = '" . $this->db->escape($transaction_id) . "' LIMIT 1");
	}

	public function getFailedTransaction($paypl_iframe_order_transaction_id) {
		$result = $this->db->query("
			SELECT *
			FROM #__paypal_iframe_order_transaction
