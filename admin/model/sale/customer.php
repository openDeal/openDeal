...
class ModelSaleCustomer extends \Core\Model {
        ...
        if (isset($data['address'])) {
            foreach ($data['address'] as $address) {
                ...
                if (isset($address['default'])) {
                    ...
                    $this->db->query("UPDATE #__customer SET address_id = '" . $address_id . "' WHERE customer_id = '" . (int) $customer_id . "'");
                }
            }
        }
    }
    ...
    public function editCustomer($customer_id, $data) {
        ...
        if (isset($data['address'])) {
            foreach ($data['address'] as $address) {
                $this->db->query("INSERT INTO #__address SET address_id = '" . (int) $address['address_id'] . "', customer_id = '" . (int) $customer_id . "', firstname = '" . $this->db->escape($address['firstname']) . "', lastname = '" . $this->db->escape($address['lastname']) . "', company = '" . $this->db->escape($address['company']) . "', company_id = '" . $this->db->escape($address['company_id']) . "', tax_id = '" . $this->db->escape($address['tax_id']) . "', address_1 = '" . $this->db->escape($address['address_1']) . "', address_2 = '" . $this->db->escape($address['address_2']) . "', city = '" . $this->db->escape($address['city']) . "', postcode = '" . $this->db->escape($address['postcode']) . "', country_id = '" . (int) $address['country_id'] . "', zone_id = '" . (int) $address['zone_id'] . "'");
    ...
    public function getAddress($address_id) {
        $address_query = $this->db->query("SELECT * FROM #__address WHERE address_id = '" . (int) $address_id . "'");

        if ($address_query->num_rows) {
            $country_query = $this->db->query("SELECT * FROM `#__country` WHERE country_id = '" . (int) $address_query->row['country_id'] . "'");
            ...
            $zone_query = $this->db->query("SELECT * FROM `#__zone` WHERE zone_id = '" . (int) $address_query->row['zone_id'] . "'");
    ...
    public function getAddresses($customer_id) {
        ...
        $query = $this->db->query("SELECT address_id FROM #__address WHERE customer_id = '" . (int) $customer_id . "'");
    ...
    public function getTotalAddressesByCustomerId($customer_id) {
        $query = $this->db->query("SELECT COUNT(*) AS total FROM #__address WHERE customer_id = '" . (int) $customer_id . "'");

        return $query->row['total'];
    }

    public function getTotalAddressesByCountryId($country_id) {
        $query = $this->db->query("SELECT COUNT(*) AS total FROM #__address WHERE country_id = '" . (int) $country_id . "'");

        return $query->row['total'];
    }

    public function getTotalAddressesByZoneId($zone_id) {
        $query = $this->db->query("SELECT COUNT(*) AS total FROM #__address WHERE zone_id = '" . (int) $zone_id . "'");
    ...
    public function addTransaction($customer_id, $description = '', $amount = '', $order_id = 0) {
        ...
        if ($customer_info) {
            $this->db->query("INSERT INTO #__customer_transaction SET customer_id = '" . (int) $customer_id . "', order_id = '" . (int) $order_id . "', description = '" . $this->db->escape($description) . "', amount = '" . (float) $amount . "', date_added = NOW()");
    ...
    public function getTransactions($customer_id, $start = 0, $limit = 10) {
        ...
        $query = $this->db->query("SELECT * FROM #__customer_transaction WHERE customer_id = '" . (int) $customer_id . "' ORDER BY date_added DESC LIMIT " . (int) $start . "," . (int) $limit);

        return $query->rows;
    }

    public function getTotalTransactions($customer_id) {
        $query = $this->db->query("SELECT COUNT(*) AS total  FROM #__customer_transaction WHERE customer_id = '" . (int) $customer_id . "'");

        return $query->row['total'];
    }

    public function getTransactionTotal($customer_id) {
        $query = $this->db->query("SELECT SUM(amount) AS total FROM #__customer_transaction WHERE customer_id = '" . (int) $customer_id . "'");

        return $query->row['total'];
    }

    public function getTotalTransactionsByOrderId($order_id) {
        $query = $this->db->query("SELECT COUNT(*) AS total FROM #__customer_transaction WHERE order_id = '" . (int) $order_id . "'");
    ...
    public function addReward($customer_id, $description = '', $points = '', $order_id = 0) {
        ...
        if ($customer_info) {
            $this->db->query("INSERT INTO #__customer_reward SET customer_id = '" . (int) $customer_id . "', order_id = '" . (int) $order_id . "', points = '" . (int) $points . "', description = '" . $this->db->escape($description) . "', date_added = NOW()");
    ...
    public function getTotalCustomerRewardsByOrderId($order_id) {
        $query = $this->db->query("SELECT COUNT(*) AS total FROM #__customer_reward WHERE order_id = '" . (int) $order_id . "'");
