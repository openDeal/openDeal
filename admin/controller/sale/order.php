...
class ControllerSaleOrder extends \Core\Controller {
    ...
    protected function getList() {
        ...
        $this->document->addScript('view/theme/default/js/plugins/datepicker/bootstrap-datepicker.js');
        $this->document->addStyle('view/theme/default/css/datepicker/datepicker.css');

        $this->template = 'sale/order_list.phtml';
        $this->children = array(
            'common/header',
            'common/footer'
        );
    ...
    public function info() {
        ...
        if ($order_info) {
            ...
            foreach ($products as $product) {
                ...
                $option = $this->model_sale_order->getOrderOptions($this->request->get['order_id'], $product['order_deal_id']);

                $shipping = $this->model_sale_order->getOrderShipping($product['order_deal_id']);

                if ($shipping) {
                    $this->data['shipping_method'] = true;
                }

                $this->data['products'][] = array(
                    'order_deal_id' => $product['order_deal_id'],
                    ...
                    'href' => $this->url->link('deal/deal/update', 'token=' . $this->session->data['token'] . '&deal_id=' . $product['deal_id'], 'SSL')
                );
            }
            ...
            $this->template = 'sale/order_info.phtml';
            $this->children = array(
                'common/header',
                'common/footer'
            );
    ...
    public function history() {
        ...
        $this->template = 'sale/order_history.phtml';
    ...
    public function invoice() {
        ...
        foreach ($orders as $order_id) {
            ...
            if ($order_info) {
                ...
                foreach ($products as $product) {
                    ...
                    $option = $this->model_sale_order->getOrderOptions($order_id, $product['order_deal_id']);
                    $shipping = $this->model_sale_order->getOrderShipping($product['order_deal_id']);


                    if($shipping){
                        $order_info['shipping_method'] = true;
                    }
        ...
        $this->template = 'sale/order_invoice.phtml';
