...
class ControllerReportExportXLS extends \Core\Controller {
    ...
    public function index() {
        ...
        $this->template = 'report/export_xls.phtml';
        $this->children = array(
            'common/header',
            'common/footer'
        );
    ...
    public function createInvoice() {
        ...
        $this->objPHPExcel = new PHPExcel();
        $this->objPHPExcel->getProperties()->setCreator("openDeal")
                ->setLastModifiedBy("openDeal")
                ->setTitle("OpenDeal Order Export")
                ->setSubject("OpenDeal Order Export")
                ->setDescription("OpenDeal Order Export")
                ->setKeywords("OpenDeal Order Export")
                ->setCategory("OpenDeal Order Export");
       ...
       // $this->objPHPExcel->getActiveSheet()->getStyle('H7')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
        ...
        foreach ($products as $prod) {
            ...
            $option = $this->model_sale_order->getOrderOptions($order_id, $prod['order_deal_id']);

            

            // Add each product line
            $this->objPHPExcel->getActiveSheet()->setCellValue('A' . $counter, html_entity_decode($prod['title'], ENT_QUOTES, 'UTF-8'));
            $this->objPHPExcel->getActiveSheet()->setCellValue('D' . $counter, html_entity_decode($option['title'], ENT_QUOTES, 'UTF-8'));
    ...
    private function createExcelWorksheet($order_id) {
        ...
        if ($this->mainCounter == 1) {
            ...
            $this->objPHPExcel->getActiveSheet()->setCellValue('K' . $this->mainCounter, $this->language->get('header_shipping_firstname'));
            $this->objPHPExcel->getActiveSheet()->setCellValue('L' . $this->mainCounter, $this->language->get('header_shipping_lastname'));
            $this->objPHPExcel->getActiveSheet()->setCellValue('M' . $this->mainCounter, $this->language->get('header_shipping_address'));
            $this->objPHPExcel->getActiveSheet()->setCellValue('N' . $this->mainCounter, $this->language->get('header_shipping_city'));
            $this->objPHPExcel->getActiveSheet()->setCellValue('O' . $this->mainCounter, $this->language->get('header_shipping_postcode'));
            $this->objPHPExcel->getActiveSheet()->setCellValue('P' . $this->mainCounter, $this->language->get('header_shipping_zone'));
        ...
        foreach ($products as $prod) {
            ...
            $option = $this->model_sale_order->getOrderOptions($order_id, $prod['order_deal_id']);
            if($prod['is_coupon'] == '1'){
                $shipping = $this->language->get('shipping_coupon');
            }elseif($prod['is_collect'] == '1'){
                $shipping = $this->language->get('shipping_collect');
            }else {
                $shipping_data = $this->model_sale_order->getOrderShipping($prod['order_deal_id']);
                $shipping = $shipping_data['title'];
            }
            $company = $this->model_sale_order->getOrderProductCompanyName($prod['order_deal_id']);
            ...
            $this->objPHPExcel->getActiveSheet()->setCellValue('J' . $counter, html_entity_decode($option['title'], ENT_QUOTES, 'UTF-8'));
        
            $this->objPHPExcel->getActiveSheet()->setCellValue('K' . $counter, $this->data['orders'][0]['shipping_firstname']);
            $this->objPHPExcel->getActiveSheet()->setCellValue('L' . $counter, $this->data['orders'][0]['shipping_lastname']);
            $this->objPHPExcel->getActiveSheet()->setCellValue('M' . $counter, $this->data['orders'][0]['shipping_address_1'] . ' ' . $this->data['orders'][0]['shipping_address_2']);
            $this->objPHPExcel->getActiveSheet()->setCellValue('N' . $counter, $this->data['orders'][0]['shipping_city']);
            $this->objPHPExcel->getActiveSheet()->setCellValue('O' . $counter, $this->data['orders'][0]['shipping_postcode']);
            $this->objPHPExcel->getActiveSheet()->setCellValue('P' . $counter, $this->data['orders'][0]['shipping_zone']);
