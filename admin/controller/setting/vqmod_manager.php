...
class ControllerSettingVQModManager extends \Core\Controller {
    ...
    public function __construct($registry) {
        ...
        // Paths and Files
        $this->base_dir = DIR_ROOT;
        $this->vqmod_dir = DIR_SYSTEM . 'vendor/vqmod/';
        $this->vqmod_script_dir = DIR_SYSTEM . 'vendor/vqmod/xml/';
        $this->vqcache_dir = DIR_SYSTEM . 'vendor/vqmod/vqcache/';
        $this->vqcache_files = DIR_SYSTEM . 'vendor/vqmod/vqcache/vq*';
        $this->vqmod_log = DIR_SYSTEM . 'vendor/vqmod/vqmod.log'; // Depricated VQMod 2.2.0
        $this->vqmod_logs_dir = DIR_SYSTEM . 'vendor/vqmod/logs/';
        $this->vqmod_logs = DIR_SYSTEM . 'vendor/vqmod/logs/*.log';
        $this->vqmod_modcache = DIR_SYSTEM . 'vendor/vqmod/mods.cache'; // VQMod 2.2.0
        $this->vqmod_opendeal_script = DIR_SYSTEM . 'vendor/vqmod/xml/vqmod_opendeal.xml';
        $this->vqmod_path_replaces = DIR_SYSTEM . 'vendor/vqmod/pathReplaces.php'; // VQMod 2.3.0

        clearstatcache();
    }

    public function index() {
        $this->language->load('setting/vqmod_manager');
        ...
        // Language
        $this->data = array_merge($this->data, $this->language->load('setting/vqmod_manager'));
        ...
        $this->data['breadcrumbs'][] = array(
            'href' => $this->url->link('setting/vqmod_manager', 'token=' . $this->session->data['token'], 'SSL'),
            'text' => $this->language->get('heading_title'),
            'separator' => ' :: '
        );

        // Action Buttons
        $this->data['action'] = $this->url->link('setting/vqmod_manager', 'token=' . $this->session->data['token'], 'SSL');
        $this->data['clear_log'] = $this->url->link('setting/vqmod_manager/clear_log', 'token=' . $this->session->data['token'], 'SSL');
        $this->data['clear_vqcache'] = $this->url->link('setting/vqmod_manager/clear_vqcache', 'token=' . $this->session->data['token'], 'SSL');
        $this->data['download_log'] = $this->url->link('setting/vqmod_manager/download_log', 'token=' . $this->session->data['token'], 'SSL');
        $this->data['download_scripts'] = $this->url->link('setting/vqmod_manager/download_vqmod_scripts', 'token=' . $this->session->data['token'], 'SSL');
        $this->data['download_vqcache'] = $this->url->link('setting/vqmod_manager/download_vqcache', 'token=' . $this->session->data['token'], 'SSL');
            ...
            foreach ($vqmod_scripts as $vqmod_script) {
                ...
                if ($extension == 'xml_') {
                    $action[] = array(
                        ...
                        'href' => $this->url->link('setting/vqmod_manager/vqmod_install', 'token=' . $this->session->data['token'] . '&vqmod=' . $file, 'SSL')
                    );
                } else {
                    $action[] = array(
                        'text' => $this->language->get('text_uninstall'),
                        'href' => $this->url->link('setting/vqmod_manager/vqmod_uninstall', 'token=' . $this->session->data['token'] . '&vqmod=' . $file, 'SSL')
                    );
                }
                ...
                $this->data['vqmods'][] = array(
                    ...
                    'delete' => $this->url->link('setting/vqmod_manager/vqmod_delete', 'token=' . $this->session->data['token'] . '&vqmod=' . basename($vqmod_script), 'SSL'),
                    'action' => $action,
        ...
        // Template
        $this->template = 'setting/vqmod_manager.phtml';
        $this->children = array(
            'common/header',
            'common/footer'
        );
    ...
    public function vqmod_install() {
        ...
        $this->redirect($this->url->link('setting/vqmod_manager', 'token=' . $this->session->data['token'], 'SSL'));
    }
    ...
    private function vqmod_installation_check() {
        ...
        if (!is_file($this->vqmod_opendeal_script)) {
            if (is_file($this->vqmod_opendeal_script . '_')) {
                $this->session->data['error'] = $this->language->get('error_opendeal_xml_disabled');
            } else {
                $this->session->data['vqmod_installation_error'] = $this->language->get('error_opendeal_xml');
                return false;
            }
        }

        // Check that OpenDeal 1.5.x is being used - other errors will appear on the page if they're using 1.4.x but at least the user will be told what the issue is
        if (!defined('VERSION') || version_compare(VERSION, '1.0.0', '<')) {
            $this->session->data['vqmod_installation_error'] = $this->language->get('error_opendeal_version');
            return false;
        }

        // If OpenDeal 1.5.4+ check that vqmod_opendeal.xml 2.1.7 or later is being used
        if (version_compare(VERSION, '1.5.4', '>=')) {
            libxml_use_internal_errors(true);
            $xml = simplexml_load_file($this->vqmod_opendeal_script);
            libxml_clear_errors();

            // In VQMod 2.3.0 'vqmver' is set as '2.X'
            if ((isset($xml->vqmver)) && (strtolower($xml->vqmver) != '2.x') && (version_compare($xml->vqmver, '2.1.7', '<'))) {
                $this->session->data['vqmod_installation_error'] = $this->language->get('error_opendeal_xml_version');
                return false;
            }
        }

        // Check if VQMod class is added to OpenDeal
        if (!class_exists('VQMod')) {
            if (is_file($this->vqmod_dir . 'install/index.php') && is_file($this->vqmod_dir . 'install/ugrsr.class.php')) {
                ...
                $this->session->data['vqmod_installation_error'] = $this->language->get('error_vqmod_opendeal_integration');
            }
            return false;
        }
        ...
        // Check if vqcache files from vqmod_opendeal.xml have been generated
        $vqcache_files = array(
            'vq2-system_Core_Controller.php',
            'vq2-system_Core_Front.php',
            'vq2-system_Core_Loader.php',
        ...
        foreach ($vqcache_files as $vqcache_file) {
            ...
            if (!is_file($this->vqcache_dir . $vqcache_file) && !is_file($this->vqmod_opendeal_script . '_')) {
                echo $this->vqcache_dir . $vqcache_file;
    ...
    private function userPermission($permission = 'modify') {
        ...
        if (!$this->user->hasPermission($permission, 'setting/vqmod_manager')) {
