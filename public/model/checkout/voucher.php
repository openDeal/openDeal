<?php
class ModelCheckoutVoucher extends \Core\Model {
	public function addVoucher($order_id, $data) {
		$this->db->query("INSERT INTO #__voucher SET order_id = '" . (int)$order_id . "', code = '" . $this->db->escape($data['code']) . "', from_name = '" . $this->db->escape($data['from_name']) . "', from_email = '" . $this->db->escape($data['from_email']) . "', to_name = '" . $this->db->escape($data['to_name']) . "', to_email = '" . $this->db->escape($data['to_email']) . "', voucher_theme_id = '" . (int)$data['voucher_theme_id'] . "', message = '" . $this->db->escape($data['message']) . "', amount = '" . (float)$data['amount'] . "', status = '1', date_added = NOW()");
	...
	public function getVoucher($code) {
		...
		$voucher_query = $this->db->query("SELECT *, vtd.name AS theme FROM #__voucher v LEFT JOIN #__voucher_theme vt ON (v.voucher_theme_id = vt.voucher_theme_id) LEFT JOIN #__voucher_theme_description vtd ON (vt.voucher_theme_id = vtd.voucher_theme_id) WHERE v.code = '" . $this->db->escape($code) . "' AND vtd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND v.status = '1'");
		
		if ($voucher_query->num_rows) {
			if ($voucher_query->row['order_id']) {
				$order_query = $this->db->query("SELECT * FROM `#__order` WHERE order_id = '" . (int)$voucher_query->row['order_id'] . "' AND order_status_id = '" . (int)$this->config->get('config_complete_status_id') . "'");
			
				if (!$order_query->num_rows) {
					$status = false;
				}
				
				$order_voucher_query = $this->db->query("SELECT * FROM `#__order_voucher` WHERE order_id = '" . (int)$voucher_query->row['order_id'] . "' AND voucher_id = '" . (int)$voucher_query->row['voucher_id'] . "'");
			...
			$voucher_history_query = $this->db->query("SELECT SUM(amount) AS total FROM `#__voucher_history` vh WHERE vh.voucher_id = '" . (int)$voucher_query->row['voucher_id'] . "' GROUP BY vh.voucher_id");
	...
	public function confirm($order_id) {
		...
		if ($order_info) {
			...
			$voucher_query = $this->db->query("SELECT *, vtd.name AS theme FROM `#__voucher` v LEFT JOIN #__voucher_theme vt ON (v.voucher_theme_id = vt.voucher_theme_id) LEFT JOIN #__voucher_theme_description vtd ON (vt.voucher_theme_id = vtd.voucher_theme_id) AND vtd.language_id = '" . (int)$order_info['language_id'] . "' WHERE v.order_id = '" . (int)$order_id . "'");
	...
	public function redeem($voucher_id, $order_id, $amount) {
		$this->db->query("INSERT INTO `#__voucher_history` SET voucher_id = '" . (int)$voucher_id . "', order_id = '" . (int)$order_id . "', amount = '" . (float)$amount . "', date_added = NOW()");
	}
}
?>